...
class S3 {
  ...
  constructor({
    ...
    bucketName,
    region = 'auto',
    maxRequestSizeInBytes = C.MIN_MAX_REQUEST_SIZE_IN_BYTES,
  ...
  private _validateConstructorParams(
    ...
    if (typeof bucketName !== 'string' || bucketName.trim().length === 0)
      throw new TypeError(C.ERROR_BUCKET_NAME_REQUIRED);
  }
  ...
  sanitizeETag = (etag: string): string => U.sanitizeETag(etag);
  ...
  setProps = (props: T.S3Config) => {
    ...
    this.maxRequestSizeInBytes = props.maxRequestSizeInBytes || C.MIN_MAX_REQUEST_SIZE_IN_BYTES;
  ...
  async getContentLength(key: string): Promise<number> {
    ...
    const encodedKey = U.uriResourceEscape(key);
    ...
    const contentLength = res.headers.get(C.HEADER_CONTENT_LENGTH);
  ...
  async createBucket(): Promise<boolean> {
    ...
    const headers = {
      ...
      [C.HEADER_AMZ_CONTENT_SHA256]: await U.hash(xmlBody),
    };
   ...
   * @returns {Promise<T.ExistResponseCode>} True if the file exists, false otherwise. 0 - Not found (404), 1 - Found (200), 2 - ETag mismatch (412).
   * @throws {TypeError} If the key is not a non-empty string.
   */
  async fileExists(key: string, opts: Record<string, any> = {}): Promise<T.ExistResponseCode> {
    this._checkKey(key);
    const { filteredOpts, conditionalHeaders } = this._filterIfHeaders(opts);
    const headers = { [C.HEADER_AMZ_CONTENT_SHA256]: C.UNSIGNED_PAYLOAD, ...conditionalHeaders };
    const encodedKey = U.uriResourceEscape(key);
    ...
    try {
      ...
      this._log('error', `${C.ERROR_PREFIX}Failed to check if file exists: ${errorMessage}`);
      throw new Error(`${C.ERROR_PREFIX}Failed to check if file exists: ${errorMessage}`);
    }
  }
  private async _sign(
    method: T.HttpMethod,
  ...
  async list(
    ...
    const headers = {
      [C.HEADER_CONTENT_TYPE]: C.JSON_CONTENT_TYPE,
      [C.HEADER_AMZ_CONTENT_SHA256]: C.UNSIGNED_PAYLOAD,
    };
    const encodedKey = delimiter === '/' ? delimiter : U.uriEscape(delimiter);
    ...
    if (method === 'HEAD') {
      const contentLength = res.headers.get(C.HEADER_CONTENT_LENGTH);
      const lastModified = res.headers.get(C.HEADER_LAST_MODIFIED);
    ...
    const data = U.parseXml(responseBody);
  ...
  async listMultiPartUploads(
    ...
    if (method === 'HEAD') {
      return {
        ...
        mtime: new Date(res.headers.get(C.HEADER_LAST_MODIFIED) ?? ''),
  ...
  async getObjectWithETag(
    ...
    try {
      ...
      return { etag: U.sanitizeETag(etag), data };
    } catch (error) {
  ...
  async put(key: string, data: string | Buffer): Promise<Response> {
    ...
    const headers = {
      [C.HEADER_CONTENT_LENGTH]: contentLength,
    };
    const encodedKey = U.uriResourceEscape(key);
   ...
   */
  async getMultipartUploadId(key: string, fileType: string = C.DEFAULT_STREAM_CONTENT_TYPE): Promise<string> {
    this._checkKey(key);
    if (typeof fileType !== 'string') {
      this._log('error', `${C.ERROR_PREFIX}fileType must be a string`);
    ...
    const responseBody = await res.text();
    const parsedResponse = U.parseXml(responseBody);
    ...
    if (
      ...
      this._log('error', `${C.ERROR_PREFIX}Failed to abort multipart upload: ${errorMessage}`);
      throw new Error(`${C.ERROR_PREFIX}Failed to abort multipart upload: ${errorMessage}`);
    }

    if (typeof parsedResponse === 'object' && parsedResponse !== null) {
      if (!parsedResponse.initiateMultipartUploadResult || !parsedResponse.initiateMultipartUploadResult.uploadId) {
        this._log('error', `${C.ERROR_PREFIX}Failed to create multipart upload: no uploadId in response`);
        throw new Error(`${C.ERROR_PREFIX}Failed to create multipart upload: Missing upload ID in response`);
      }

      return parsedResponse.initiateMultipartUploadResult.uploadId;
    } else {
      this._log('error', `${C.ERROR_PREFIX}Failed to create multipart upload: unexpected response format`);
      throw new Error(`${C.ERROR_PREFIX}Failed to create multipart upload: Unexpected response format`);
    }
  }
  ...
  async uploadPart(
    ...
    const headers = {
      [C.HEADER_CONTENT_LENGTH]: data.length,
    } as { [key: string]: any };
    ...
    const ETag = U.sanitizeETag(res.headers.get('etag') || '');
  ...
  async completeMultipartUpload(
    ...
    if (!Array.isArray(parts) || parts.length === 0) {
      this._log('error', C.ERROR_PARTS_REQUIRED);
      throw new TypeError(C.ERROR_PARTS_REQUIRED);
    }
    if (!parts.every(part => typeof part.partNumber === 'number' && typeof part.ETag === 'string')) {
      this._log('error', C.ERROR_INVALID_PART);
      throw new TypeError(C.ERROR_INVALID_PART);
    }
  ...
  async abortMultipartUpload(key: string, uploadId: string): Promise<object> {
    ...
    try {
      ...
      if (res.ok) {
        ...
        this._log('error', `${C.ERROR_PREFIX}Abort request failed with status ${res.status}`);
        throw new Error(`${C.ERROR_PREFIX}Abort request failed with status ${res.status}`);
      }
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      this._log('error', `${C.ERROR_PREFIX}Failed to abort multipart upload for key ${key}: ${errorMessage}`);
      throw new Error(`${C.ERROR_PREFIX}Failed to abort multipart upload for key ${key}: ${errorMessage}`);
    }
  }
